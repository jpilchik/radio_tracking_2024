...
receiver_data_all <- receiver_data_all %>%
  mutate(
    tagID = case_when(
      tagID == 30 & date >= cutoff_date ~ 30.1,
      tagID == 39 & date >= cutoff_date ~ 39.1,
      tagID == 46 & date >= cutoff_date ~ 46.1,
      tagID == 29 & date >= cutoff_date ~ 29.1,
      TRUE ~ as.numeric(tagID)  # Keep the original radioID for other cases
    )
  )
...
# Convert all radioID values to numeric with one decimal place
tracking_data_all <- tracking_data_all %>%
  mutate(radioID = format(as.numeric(radioID), nsmall = 1))
