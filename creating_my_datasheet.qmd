...
# Manually set the correct date for AMETHYST day and night rows
flow_tracker_1 <- flow_tracker_1 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-06-11 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-06-11 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_2 <- flow_tracker_2 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-06-18 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-06-19 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_3 <- flow_tracker_3 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-06-26 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-06-26 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_4 <- flow_tracker_4 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-07-02 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-07-02 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_5 <- flow_tracker_5 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-07-11 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-07-11 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_6 <- flow_tracker_6 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-07-17 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-07-17 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_7 <- flow_tracker_7 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-07-25 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-07-25 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_8 <- flow_tracker_8 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-07-31 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-07-31 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_9 <- flow_tracker_9 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-08-08 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-08-08 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
flow_tracker_10 <- flow_tracker_10 %>%
  mutate(ftTime_EST = case_when(
    river == "AMETHYST" & shift == "day" ~ as.POSIXct("2024-08-13 00:00:00", tz="EST"),  # Set the correct date for the day row
    river == "AMETHYST" & shift == "night" ~ as.POSIXct("2024-08-13 00:00:00", tz="EST"),  # Set the correct date for the night row
    TRUE ~ ftTime_EST  # Keep the existing dateTime for other rows
  ))
...
amethyst_combined <- flow_tracker_11 %>%
  filter(river %in% c("BUFFAM", "HARRIS")) %>%   # Filter BUFFAM and HARRIS
  mutate(date = as.Date(ftTime_EST)) %>%          # Extract only the date portion
  group_by(date, shift) %>%  # Group by date and shift
  summarise(
    river = "AMETHYST",                          # Set river to AMETHYST
    ftDischarge_cfs = sum(ftDischarge_cfs, na.rm = TRUE), # Sum ftDischarge_cfs
    .groups = "drop"
  ) %>%
  mutate(ftTime_EST = as.POSIXct(paste(date, "00:00:00"), format="%Y-%m-%d %H:%M:%S", tz = "EST"))  # Convert date back to POSIXct for consistency
...
for (file_name in flow_file_names) {
  ...
  flow_data <- raw_flow_data %>%
    mutate(ftDate = as.Date(ftTime_EST)) %>%  # Extract date from ftTime_EST
    
    # Loop through each dataset and apply the transformations
    group_by(shift, river, ftDate) %>%
...
flow_tracker_6_filtered <- flow_tracker_6 %>%
  mutate(date = as.Date(ftTime_EST)) %>%  # Extract just the date from dateTime
  filter(!(river == "DICKEY" & date == as.Date("2024-07-16") & shift == "night"))

# Select specific columns
flow_tracker_6_filtered <- flow_tracker_6_filtered %>%
    select(river, ftDischarge_cfs, ftTime_EST, shift) 

# Write new flow csv
write.csv(flow_tracker_6_filtered, 
          file = "C:/Users/jpilchik/OneDrive - DOI/Documents/My study plan/Tagging/Tracking/radio_tracking_2024/data/flow_tracker_data/flow_tracker_6_filtered.csv", 
          row.names = FALSE)
...
# Convert dateTime to date format
flow_tracker_9_filtered <- flow_tracker_9 %>%
  mutate(date = as.Date(ftTime_EST)) %>%  # Extract just the date from dateTime
...
# Select specific columns
flow_tracker_9_filtered <- flow_tracker_9_filtered %>%
    select(river, ftDischarge_cfs, ftTime_EST, shift) 

# Write new flow csv
write.csv(flow_tracker_9_filtered, 
          file = "C:/Users/jpilchik/OneDrive - DOI/Documents/My study plan/Tagging/Tracking/radio_tracking_2024/data/flow_tracker_data/flow_tracker_9_filtered.csv", 
          row.names = FALSE)
...
flow_tracker_6_filtered <- read.csv("C:/Users/jpilchik/OneDrive - DOI/Documents/My study plan/Tagging/Tracking/radio_tracking_2024/data/flow_tracker_data/flow_tracker_6_filtered.csv")

# Combine the datasets by shift and river columns
fish_flow_6 <- left_join(fish_survey_6, flow_tracker_6_filtered, by = c("shift", "river"))
...
flow_tracker_9_filtered <- read.csv("C:/Users/jpilchik/OneDrive - DOI/Documents/My study plan/Tagging/Tracking/radio_tracking_2024/data/flow_tracker_data/flow_tracker_9_filtered.csv")

# Combine the datasets by shift and river columns
fish_flow_9 <- left_join(fish_survey_9, flow_tracker_9_filtered, by = c("shift", "river"))
...
# Ensure both date columns are in Date format before joining
fish_survey_11$date <- as.Date(fish_survey_11$date)
flow_tracker_11$date <- as.Date(flow_tracker_11$ftTime_EST)

# Combine the datasets by shift and river columns
fish_flow_11 <- left_join(fish_survey_11, flow_tracker_11, by = c("shift", "river", "date"))
...
### Lotek receiver 000901 data
No ID+GPS data on this receiver - No combination needed

```{r processing the lotek receiver 000901 ID only data}

# ID Only Data ###########################################################

# Read the dataset
receiver_000901_raw_data <- read.delim("C:/Users/jpilchik/OneDrive - DOI/Documents/My study plan/Tagging/Tracking/rawLotekData/000901/20250224/000901_20250224.txt")

# Rename the single column in the dataset as "columnOne"
names(receiver_000901_raw_data) <- c("columnOne")

# Detect the start of the row that says "ID Only Records:" and create a flag
receiver_000901_ID_data <- receiver_000901_raw_data %>% 
  mutate(column2 = ifelse(columnOne == "ID Only Records:", 1, 0)) 

# Detect the start of row that says "End of Data" 
end_of_data_row <- which(receiver_000901_raw_data$columnOne == "End of Data")

# If "End of Data" is found, slice until the row before it
if(length(end_of_data_row) > 0) {
  end_of_data_row <- end_of_data_row[1] - 1  # One row before "End of data"
} else {
  end_of_data_row <- nrow(receiver_000901_raw_data)  # If "End of data" not found, slice till the bottom
}

# Slice out the data by detecting the start and extracting relevant rows
receiver_000901_ID_data <- receiver_000901_ID_data %>% 
  slice((which(grepl(1, receiver_000901_ID_data$column2)) + 2) : end_of_data_row)
...
# Combine the date and time into a DateTime column
receiver_000901_ID_data$trackedTime_EST <- as.POSIXct(paste(receiver_000901_ID_data$Date, 
                                                         receiver_000901_ID_data$Time), 
                                                   format="%m/%d/%y %H:%M:%S",
                                                   tz = "EST") 
...
# Ensure the 'date' column is in Date format
receiver_000901_ID_data$date <- ymd(receiver_000901_ID_data$date)
...
# Clean workspace by removing excess variables
rm(receiver_000901_raw_data, river_date_ranges)

# View dataframe
head(receiver_000901_ID_data)
...
# Combine the three final dataframes horizontally
receiver_data_all <- bind_rows(receiver_000900_data, receiver_000517_ID_data, receiver_000901_ID_data)

# Clean workspace by removing excess variables
rm(receiver_000900_data, receiver_000517_ID_data, receiver_000901_ID_data)
...
for (file_name in stream_file_names) {
  ...
  stream_data <- raw_stream_data %>%
    ...
    # Step 2: Select specific columns
    select(date, stream, airTemp, cloud, precip, startTime, endTime, iso, Notes, 
...
fish_flow_receiver_stream_1 <- left_join(fish_flow_receiver_1, 
                           stream_survey_1, 
                           by = c("shift", "riverMatch" = "river", "date"))
...
```{r}
# Filter to keep rows where 'source' is not NA
duplicates <- filter(tracking_data_all, !is.na(source))
