...
ggplot(tracking_data_locations_filtered, aes(x = river, y = movement_rate)) + 
  ...
  labs(
    x = "River",
    y = "Movement Rate (meters/hour)",
    title = "Movement Rate by River"
  )

```
```{r}
#Boxplot of movement rate (m/hour) 2.0

tracking_data_locations_filtered$river_combined <- tracking_data_locations_filtered$river

tracking_data_locations_filtered$river_combined[tracking_data_locations_filtered$river %in% c("AMETHYST", "BUFFAM", "HARRIS")] <- "AMETHYST"

tracking_data_locations_filtered$river_combined <- as.factor(tracking_data_locations_filtered$river_combined)

ggplot(tracking_data_locations_filtered, aes(x = river_combined, y = movement_rate)) + 
  geom_boxplot() +
  labs(
    x = "River",
    y = "Movement Rate (meters/hour)",
...
```
```{r}
tracking_data_locations_16 <- tracking_data_locations_filtered %>% 
  filter(radioID == "16")

tracking_data_locations_16 <- tracking_data_locations_16 %>% 
  rename(dateTime = trackedTime_EST)
```
```{r}
# Find max values to scale distance to temperature range
max_temp <- max(tag16_323$temp, na.rm = TRUE)
max_dist <- max(tracking_data_locations_16$movement_rate, na.rm = TRUE)

# Scale distance_moved to fit the temperature range (for visualization only)
tracking_data_locations_16$movement_rate_scaled <- tracking_data_locations_16$movement_rate * (max_temp / max_dist)

# Create the dual-axis plot
ggplot() +
  # Temperature line (Left y-axis)
  geom_line(data = tag16_323, aes(x = dateTime, y = temp, color = "Temperature"), size = 1) +
  
  # Distance moved line (Right y-axis, rescaled)
  geom_line(data = tracking_data_locations_16, aes(x = dateTime, y = movement_rate_scaled, color = "Movement Rate"), size = 1, linetype = "dashed") +
  
  # Define the primary and secondary y-axes
  scale_y_continuous(
    name = "Temperature (°C)",
    sec.axis = sec_axis(~ . * (max_dist / max_temp), name = "Movement Rate (m/h)")  # Rescale back
  ) +
  
  # Color settings
  scale_color_manual(values = c("Temperature" = "red", "Movement Rate" = "green")) +
  
  # Labels and theme
  labs(title = "Fish 16 Internal Temperature and Movement Rate Over Time",
       x = "DateTime",
       color = "Legend") +
  theme_minimal()
```
...
ggplot() +
  ...
  # Distance moved line (Right y-axis, rescaled)
  geom_smooth(data = tracking_data_locations_16, aes(x = dateTime, y = movement_rate_scaled), color = "green", method = "loess", span = 0.3, se = FALSE) +
  ...
  # Temperature gradient color scale
  scale_color_gradientn(colors = c("blue", "purple", "red"), name = "Temperature (°C)") +
...
# Find max values to scale distance to temperature range
max_temp <- max(tag16_323$temp, na.rm = TRUE)
max_dist <- max(tracking_data_locations_16$distMoved_m, na.rm = TRUE)

# Scale distance_moved to fit the temperature range (for visualization only)
tracking_data_locations_16$distance_scaled <- tracking_data_locations_16$distMoved_m * (max_temp / max_dist)
...
ggplot() +
  ...
  # Distance moved line (Right y-axis, rescaled)
  geom_line(data = tracking_data_locations_16, aes(x = dateTime, y = distance_scaled), color = "green", size = 1) +
  
  # Define the primary and secondary y-axes
  scale_y_continuous(
    limits = c(0, 25),
    name = "Temperature (°C)",
    sec.axis = sec_axis(~ . * (max_dist / max_temp), name = "Distance Moved (m)")  # Rescale back
  ) +
  ...
  # Labels and theme
  labs(title = "Fish 16 Internal Temperature and Distance Moved Over Time",
       x = "DateTime") +
  theme_minimal()
```
...
move_temp_16 <- ggplot() +
  ...
  # Distance moved line (Right y-axis, rescaled)
  geom_smooth(data = tracking_data_locations_16, aes(x = dateTime, y = distance_scaled), color = "black", method = "loess", span = 0.27, se = FALSE) +
  
  # Define the primary and secondary y-axes
  scale_y_continuous(
    limits = c(0, 25),
    name = "Temperature (°C)",
    sec.axis = sec_axis(~ . * (max_dist / max_temp), name = "Distance Moved (m)")
  ) +
  
  # Temperature gradient color scale
  scale_color_gradient(low = "blue", high = "red", limits = c(colorMin, colorMax), name = "temp") +  # Set colors for low and high temperatures
  
  # Labels and theme
  labs(title = "Fish 16's Internal Temperature and Movement Distance Over Time",
       x = "DateTime") +
  theme_minimal()
...
ggsave("C:/Users/jpilchik/OneDrive - DOI/Documents/My study plan/Tagging/Tracking/radio_tracking_2024/plots/move_temp_16.png",
       arrangeGrob(move_temp_16, nrow = 1, ncol = 1), 
       width = 10, height = 15)  # Adjust width and height as needed
```
```{r}
# Find the closest temperature record for each fish movement point
merged_data <- tracking_data_locations_16 %>%
  rowwise() %>%
  mutate(temp = tag16_323$temp[which.min(abs(difftime(tag16_323$dateTime, trackedTime_EST, units="mins")))])

# Plot movement vs. temperature over time
ggplot(merged_data, aes(x = trackedTime_EST, y = distMoved_m, color = temp)) +
  geom_line() +
  scale_color_gradient(low = "blue", high = "red") + # Adjust color scale
  labs(title = "Fish Movement vs. Temperature Over Time",
       x = "DateTime",
       y = "Distance Moved (m)",
       color = "Temperature (°C)") +
  theme_minimal()
