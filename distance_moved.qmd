...
4. Best Approach for Your Case
If you need fine-scale movement tracking (stream edge to center):
-Use snap_x, snap_y, and snapdist for within-stream movements.
-Calculate Euclidean distance between consecutive snap_x, snap_y points:
```{r}
tracking_data_locations <- tracking_data_locations %>%
  group_by(radioID) %>%
  arrange(trackedTime_EST) %>%
  mutate(euclidean_movement = sqrt((snap_x - lag(snap_x))^2 + (snap_y - lag(snap_y))^2)) %>%
  ungroup()

head(tracking_data_locations)

```
-Compare Euclidean distance vs. riverdistance() results.

If you only need general river movements:
-Stick with seg and vert since they align with river structure.

Final Thoughts
-If your river network has a high number of vertices, seg and vert may work well.
-If you need sub-meter movement precision, use snap_x, snap_y, and snapdist instead.
