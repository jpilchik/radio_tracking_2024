...
```{r read in the data}
# Read in the data
tracking_data_filtered <- read.csv("C:/Users/jpilchik/OneDrive - DOI/Documents/My study plan/Tagging/Tracking/radio_tracking_2024/data/tracking_data_filtered.csv")
```
...
iPad_data <- tracking_data_filtered %>%
...
filtered_data <- tracking_data_filtered %>%
...
habitat_summary <- iPad_data %>%
  group_by(habitat) %>%
  summarise(mean_discharge = mean(totalDischarge, na.rm = TRUE))
...
habitat_summary <- iPad_data %>%
  group_by(habitat) %>%
  summarise(median_discharge = median(totalDischarge, na.rm = TRUE)) 
...
fish_with_two_lengths <- tracking_data_filtered %>%
...
fish_with_two_weights <- tracking_data_filtered %>%
...
```{r weight growth linear model plot}

# Separate initial and final weights
fish_weights <- tracking_data_filtered %>%
  filter(!is.na(radioID)) %>%   # Ensure there are valid (non-NA) radioID values
  group_by(radioID) %>%
  filter(!is.na(weight)) %>%  # Ensure there are valid (non-NA) weight values
  arrange(date) %>%           # Ensure the data is sorted by date so that we get the correct initial and final measurements
  mutate(initial_weight = first(weight),   # The first weight recorded for each fish
         final_weight = last(weight)) %>%  # The last weight recorded for each fish
  filter(n() == 2)  # Only keep fish with exactly two weight measurements

# Create the plot with initial vs final weight
...
```{r weight growth residuals plot}

# Fit a linear model for residuals
weights_lm_fit <- lm(final_weight ~ initial_weight, data = fish_weights)

# Compute residuals and plot them
weights_residuals_data <- augment(weights_lm_fit)

# Plot residuals
weights_residuals_plot <- ggplot(weights_residuals_data, 
...
print(weights_residuals_plot)

```

```{r}

# Fit a linear model for final weight based on initial weight
weights_lm_fit <- lm(final_weight ~ initial_weight, data = fish_weights)
...
weights_residuals_data$initial_weight <- fish_weights$initial_weight

# Plot residuals against initial weight to see if there's a pattern
weights_residuals_by_size <- ggplot(weights_residuals_data, 
                                    aes(x = initial_weight, y = .resid)) +
  ...
  labs(x = "Initial Weight (g)", y = "Residuals", 
       title = "Residuals vs Initial Weight (Size at Tagging)") +
  theme_minimal()

# Display the residuals plot
print(weights_residuals_by_size)

```
#### Lengths

```{r length growth linear model plot}

# Separate initial and final lengths
fish_lengths <- tracking_data_filtered %>%
  filter(!is.na(radioID)) %>%   # Ensure there are valid (non-NA) radioID values
  group_by(radioID) %>%
  filter(!is.na(length)) %>%  # Ensure there are valid (non-NA) length values
  arrange(date) %>%           # Ensure the data is sorted by date so that we get the correct initial and final measurements
  mutate(initial_length = first(length),   # The first length recorded for each fish
         final_length = last(length)) %>%  # The last length recorded for each fish
  filter(n() == 2)  # Only keep fish with exactly two length measurements

# Create the plot with initial vs final length
...
```{r length growth residuals plot}

# Fit a linear model for residuals
lengths_lm_fit <- lm(final_length ~ initial_length, data = fish_lengths)

# Compute residuals and plot them
lengths_residuals_data <- augment(lengths_lm_fit)

# Plot residuals
lengths_residuals_plot <- ggplot(lengths_residuals_data, 
                                 aes(x = initial_length, y = .resid)) +
  geom_point(size = 3) +  # Plot the residuals
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # Add a dashed line at 0
  labs(x = "Initial Length (mm)", y = "Residuals", title = "Residuals (Length)") +
  xlim(150, 190) +  # Set x-axis range from 150 to 190
  theme_minimal()

# Display the residuals plot
print(lengths_residuals_plot)

```

```{r}

# Fit a linear model for final length based on initial length
lengths_lm_fit <- lm(final_length ~ initial_length, data = fish_lengths)
...
lengths_residuals_data$initial_length <- fish_lengths$initial_length

# Plot residuals against initial length to see if there's a pattern
lengths_residuals_by_size <- ggplot(lengths_residuals_data, 
                                    aes(x = initial_length, y = .resid)) +
  geom_point(size = 3) +  # Plot residuals as points
  geom_smooth(method = "lm", color = "blue") +  # Add a trend line to see if residuals vary by initial size
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # Add a reference line at 0
  labs(x = "Initial Length (mm)", y = "Residuals", 
       title = "Residuals vs Initial Length (Size at Tagging)") +
  theme_minimal()

# Display the residuals plot
print(lengths_residuals_by_size)
...
```{r weight growth linear model plot (combined tags only)}

# Separate initial and final weights
fish_weights_ctags <- tracking_data_filtered %>%
  filter(!is.na(radioID)) %>%   # Ensure there are valid (non-NA) radioID values
  group_by(radioID) %>%
  filter(!is.na(weight)) %>%  # Ensure there are valid (non-NA) weight values
  arrange(date) %>%           # Ensure the data is sorted by date so that we get the correct initial and final measurements
...
weight_plot_ctags <- ggplot(fish_weights_ctags, aes(x = initial_weight, y = final_weight)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "blue") +  # Add best fit line (linear model)
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +  # Add 1:1 line
  labs(x = "Initial Weight (g)", y = "Final Weight (g)", title = "Initial vs Final Weight - Combined Tags Only") +
  xlim(40, 90) +  # Set x-axis range from 40 to 90
  ylim(40, 90) +  # Set y-axis range from 40 to 90
  theme_minimal()
...
```{r weight growth residuals plot (combined tags only)}

# Fit a linear model for residuals
weights_lm_fit_ctags <- lm(final_weight ~ initial_weight, data = fish_weights_ctags)

# Compute residuals and plot them
weights_residuals_data_ctags <- augment(weights_lm_fit_ctags)

# Plot residuals
weights_residuals_plot_ctags <- ggplot(weights_residuals_data_ctags, 
                                 aes(x = initial_weight, y = .resid)) +
  geom_point(size = 3) +  # Plot the residuals
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # Add a dashed line at 0
  labs(x = "Initial Weight (g)", y = "Residuals", title = "Residuals (Weight) - Combined Tags Only") +
  xlim(40, 90) +  # Set x-axis range from 40 to 90
  theme_minimal()

# Display the residuals plot
print(weights_residuals_plot_ctags)
...
# Add the initial_weight back to the residuals_data for plotting
weights_residuals_data_ctags$initial_weight <- fish_weights_ctags$initial_weight

# Plot residuals against initial weight to see if there's a pattern
weights_residuals_by_size_ctags <- ggplot(weights_residuals_data_ctags, 
                                    aes(x = initial_weight, y = .resid)) +
  geom_point(size = 3) +  # Plot residuals as points
  geom_smooth(method = "lm", color = "blue") +  # Add a trend line to see if residuals vary by initial size
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # Add a reference line at 0
  labs(x = "Initial Weight (g)", y = "Residuals", 
       title = "Residuals vs Initial Weight (Size at Tagging) - Combined Tags Only") +
  theme_minimal()

# Display the residuals plot
print(weights_residuals_by_size_ctags)

```
##### Lengths

```{r length growth linear model plot (combined tags only)}

# Separate initial and final lengths
fish_lengths_ctags <- tracking_data_filtered %>%
  filter(!is.na(radioID)) %>%   # Ensure there are valid (non-NA) radioID values
  group_by(radioID) %>%
  filter(!is.na(length)) %>%  # Ensure there are valid (non-NA) length values
  arrange(date) %>%           # Ensure the data is sorted by date so that we get the correct initial and final measurements
...
# Compute residuals and plot them
lengths_residuals_data_ctags <- augment(lengths_lm_fit_ctags)

# Plot residuals
lengths_residuals_plot_ctags <- ggplot(lengths_residuals_data_ctags, 
                                 aes(x = initial_length, y = .resid)) +
  geom_point(size = 3) +  # Plot the residuals
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # Add a dashed line at 0
...
# Display the residuals plot
print(lengths_residuals_plot_ctags)
...
# Add the initial_length back to the residuals_data for plotting
lengths_residuals_data_ctags$initial_length <- fish_lengths_ctags$initial_length

# Plot residuals against initial length to see if there's a pattern
lengths_residuals_by_size_ctags <- ggplot(lengths_residuals_data_ctags, 
                                    aes(x = initial_length, y = .resid)) +
  geom_point(size = 3) +  # Plot residuals as points
  geom_smooth(method = "lm", color = "blue") +  # Add a trend line to see if residuals vary by initial size
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") +  # Add a reference line at 0
...
# Display the residuals plot
print(lengths_residuals_by_size_ctags)
...
# Adjust titles and text wrapping in individual plots
weights_residuals_plot <- weights_residuals_plot + 
  theme(plot.title = element_text(size = 10, lineheight = 0.9))  # Adjust size and add wrapping
...
# Adjust titles and text wrapping in individual plots
lengths_residuals_plot <- lengths_residuals_plot + 
  theme(plot.title = element_text(size = 10, lineheight = 0.9))  # Adjust size and add wrapping
...
# Adjust titles and text wrapping in individual plots
weights_residuals_plot_ctags <- weights_residuals_plot_ctags + 
...
# Adjust titles and text wrapping in individual plots
lengths_residuals_plot_ctags <- lengths_residuals_plot_ctags + 
...
growth_lm_res <- grid.arrange(weight_plot, weights_residuals_plot, 
                              length_plot, lengths_residuals_plot, 
                              weight_plot_ctags, weights_residuals_plot_ctags, 
                              length_plot_ctags, lengths_residuals_plot_ctags, 
                              nrow = 2, ncol = 4)
...
```{r instantaneous weight growth rates all fish}

# Filter for fish with exactly two weight measurements (initial and final)
fish_weights <- tracking_data_filtered %>%
  filter(!is.na(radioID)) %>%
  group_by(radioID) %>%
  filter(!is.na(weight)) %>%
  filter(n() == 2)  # Keep fish with exactly two weight measurements

# Separate initial and final weights and dates
...
# Convert initial and final weights from grams to milligrams
fish_weights <- fish_weights %>%
...
```{r instantaneous weight growth rates combined tags only}
...
```{r instantaneous length growth rates all fish}

# Filter for fish with exactly two length measurements (initial and final)
fish_lengths <- tracking_data_filtered %>%
  filter(!is.na(radioID)) %>%
  group_by(radioID) %>%
  filter(!is.na(length)) %>%
  filter(n() == 2)  # Keep fish with exactly two length measurements

# Separate initial and final lengths and dates
fish_lengths <- fish_lengths %>%
  arrange(radioID, date) %>%
  mutate(initial_length = first(length), 
...
```{r instantaneous length growth rates combined tags only}
