...
```
```{r}
underhill_flowlines_trim <- trimriver(trimto=c(13,6,40,18,25,44,12), rivers=underhill_flowlines_UTMZONE18N)
plot(x=underhill_flowlines_trim)

# Adding vertices every 1 meter to the river network
underhill_flowlines_split <- addverts(underhill_flowlines_trim, mindist=1)
plot(x=underhill_flowlines_split)

# Checking connectedness
topologydots(rivers=underhill_flowlines_split)

# Setting the MOUTH of your watershed allows you to look at directional movement. The "seg = " value will be the segment that includes your mouth, which you can observe in the plot function above. In Underhill, it's segment 46. The "vert = " value identifies the correct node of that segment. Using "showends" on the segment you want will show you what vertex to select.
showends(seg = 6, rivers = underhill_flowlines_split) # identifies the true mouth of your watershed
underhill_mouth <- setmouth(seg = 6, vert = 452, rivers = underhill_flowlines_split) # sets the mouth to the segment and vertex you need.

# Cleaning up river network
#underhill_flowlines_fixed <- cleanup(underhill_flowlines_split)  # fixing many problems
...
res_underhill <- res_underhill %>% 
  rename(POINT_X = coords.x1, POINT_Y = coords.x2) %>%  # Rename coords.x1 and coords.x2
  mutate(Rownumber =row_number())
underhill_segvert <- xy2segvert(x = res_underhill$POINT_X, y = res_underhill$POINT_Y, rivers = underhill_flowlines_split) # change the "res$" to your X and Y coord columns
  
## Display raw points (red dots) and snapped points (blue). Run these three lines all at once
plot(x= underhill_flowlines_split)
points(res_underhill$POINT_X, res_underhill$POINT_Y, pch=16, col="red") # shows raw points
riverpoints(seg = underhill_segvert$seg, vert = underhill_segvert$vert, rivers = underhill_flowlines_split, pch = 15, col="blue")
  ...
  if (nrow(fish_data) > 1) {
    for (i in 2:nrow(fish_data)) {
      ...
      dist <- riverdistance(startseg = from_seg, startvert = from_vert, 
                            ...
                            rivers = underhill_flowlines_split, stopiferror = FALSE)
...
underhill_locations <- underhill_locations %>%
  group_by(radioID) %>%
  arrange(trackedTime_EST, .by_group = TRUE) %>%
  mutate(observation = row_number()) %>%
  ungroup()

underhill_seqbysurvey <- riverdistanceseq(unique=underhill_locations$radioID, survey=underhill_locations$observation,
      seg=underhill_locations$seg, vert=underhill_locations$vert, rivers=underhill_flowlines_split)
underhill_seqbysurvey
plotseq(underhill_seqbysurvey)
```
...
for (fish in unique(dickey_locations$radioID)) {
  ...
  if (nrow(fish_data) > 1) {
    for (i in 2:nrow(fish_data)) {
      ...
      dist <- riverdistance(startseg = from_seg, startvert = from_vert, 
                            endseg = to_seg, endvert = to_vert, 
                            rivers = dickey_flowlines_UTMZONE18N, stopiferror = FALSE)
...
for (fish in unique(dry_locations$radioID)) {
  ...
  if (nrow(fish_data) > 1) {
    for (i in 2:nrow(fish_data)) {
      ...
      dist <- riverdistance(startseg = from_seg, startvert = from_vert, 
                            endseg = to_seg, endvert = to_vert, 
                            rivers = dry_flowlines_UTMZONE18N, stopiferror = FALSE)
...
for (fish in unique(amethyst_locations$radioID)) {
  ...
  if (nrow(fish_data) > 1) {
    for (i in 2:nrow(fish_data)) {
      ...
      dist <- riverdistance(startseg = from_seg, startvert = from_vert, 
                            endseg = to_seg, endvert = to_vert, 
                            rivers = amethyst_flowlines_UTMZONE18N, stopiferror = FALSE)
...
```
```{r calculate euclidean distance moved for finer-scale movements}
# Arrange data by fish ID and datetime to ensure chronological order
amethyst_locations <- amethyst_locations %>%
  arrange(radioID, trackedTime_EST) %>%
  group_by(radioID) %>%
  mutate(
    euclideanMoved_m = sqrt((snap_x - lag(snap_x))^2 + (snap_y - lag(snap_y))^2)
  ) %>%
  ungroup()

# Replace NAs (first observation for each fish) with 0
amethyst_locations$euclideanMoved_m[is.na(amethyst_locations$euclideanMoved_m)] <- 0

# View results
head(amethyst_locations)

# This method works great for short distances, but if fish stay in one place, GPS noise might create false movement. Filter out small movements to account for GPS error?

# Include Time Between Movements
amethyst_locations <- amethyst_locations %>%
  arrange(radioID, trackedTime_EST) %>%
  group_by(radioID) %>%
  mutate(
    time_diff_hours = as.numeric(difftime(trackedTime_EST, lag(trackedTime_EST), units = "hours")),
    movement_rate = ifelse(time_diff_hours > 0, euclideanMoved_m / time_diff_hours, NA)
  ) %>%
  ungroup()
...
write.csv(tracking_data_locations, 
          "C:/Users/jpilchik/OneDrive - DOI/Documents/My study plan/Tagging/Tracking/radio_tracking_2024/data/combined_data/tracking_data_locations.csv", 
          row.names = FALSE)
```
...
unique(tracking_data_locations_filtered$river)

glimpse(tracking_data_locations_filtered)

colnames(tracking_data_locations_filtered)

ggplot(tracking_data_locations, aes(x = river, y = movement_rate)) + 
  geom_boxplot()

ggplot(data = tracking_data_locations_filtered, aes(x = river, y = movement_rate, fill = river)) +
  geom_boxplot() +
  scale_fill_manual('River', values = river, labels = river) +
  labs(title = "Movement Rate (m/hour) by River",
       y = "Movement rate (m/hour)", x = "River") +
  theme_classic()

ggplot(data = tracking_data_locations, aes(x = river, y = movement_rate, fill = river)) +
  geom_boxplot() +
  scale_fill_manual('River', values = river, labels = river) +
  labs(title = "Movement Rate (m/hour) by River",
       y = "Movement rate (m/hour)", x = "River") +
...
```{r}
# Grouping habitat by locations using snap_x and snap_y
habitat_summary <- tracking_data_locations %>%
  group_by(snap_x, snap_y) %>%
...
summary(amethyst_flowlines_UTMZONE18N)  # See number of segments and vertices
summary(dickey_flowlines_UTMZONE18N)  # See number of segments and vertices
summary(dry_flowlines_UTMZONE18N)  # See number of segments and vertices
summary(underhill_flowlines_UTMZONE18N)  # See number of segments and vertices
```
...
amethyst_vertex_counts <- sapply(amethyst_flowlines_UTMZONE18N$cumuldist, length)

# Summary statistics of vertex counts
summary(amethyst_vertex_counts)

# Check number of vertices per segment
underhill_vertex_counts <- sapply(underhill_flowlines_UTMZONE18N$cumuldist, length)

# Summary statistics of vertex counts
summary(underhill_vertex_counts)

# Check number of vertices per segment
dickey_vertex_counts <- sapply(dickey_flowlines_UTMZONE18N$cumuldist, length)

# Summary statistics of vertex counts
summary(dickey_vertex_counts)

# Check number of vertices per segment
dry_vertex_counts <- sapply(dry_flowlines_UTMZONE18N$cumuldist, length)
...
amethyst_vertex_spacing <- sapply(amethyst_flowlines_UTMZONE18N$cumuldist, function(x) if (length(x) > 1) mean(diff(x)) else NA)

# Summary of spacing
summary(amethyst_vertex_spacing)

# Compute spacing between vertices within each segment
underhill_vertex_spacing <- sapply(underhill_flowlines_UTMZONE18N$cumuldist, function(x) if (length(x) > 1) mean(diff(x)) else NA)

# Summary of spacing
summary(underhill_vertex_spacing)

# Compute spacing between vertices within each segment
dickey_vertex_spacing <- sapply(dickey_flowlines_UTMZONE18N$cumuldist, function(x) if (length(x) > 1) mean(diff(x)) else NA)

# Summary of spacing
summary(dickey_vertex_spacing)

# Compute spacing between vertices within each segment
dry_vertex_spacing <- sapply(dry_flowlines_UTMZONE18N$cumuldist, function(x) if (length(x) > 1) mean(diff(x)) else NA)
