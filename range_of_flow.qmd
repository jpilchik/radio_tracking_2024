...
flow_tracker_range <- flow_tracker_all %>%
...
discharge_ranges_ft <- flow_tracker_range %>%
  ...
  summarize(
    min_discharge = min(totalDischarge, na.rm = TRUE),  # Find the minimum value
    max_discharge = max(totalDischarge, na.rm = TRUE)   # Find the maximum value
  )
...
discharge_percentiles_ft <- flow_tracker_range %>%
  group_by(river) %>%  # Group by stream
  summarize(
    p2_5 = quantile(totalDischarge, probs = 0.025, na.rm = TRUE),  # 2.5th percentile
    p97_5 = quantile(totalDischarge, probs = 0.975, na.rm = TRUE)  # 97.5th percentile
  )
...
flow_tracker_range$date <- as.Date(flow_tracker_range$flowTime_EST)
